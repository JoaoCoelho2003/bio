<template>
  <div class="min-h-screen bg-black overflow-hidden relative">
    ...
    >
      <div class="terminal-container w-[90%] max-w-3xl mx-auto">
        ...
        <div class="terminal-body">
          ...
          >
            <span class="text-green-500">root@system</span>:<span
              class="text-blue-500"
              >~</span
            >$ Initializing system access...
            <br />
            <span v-if="showWelcome"
              >Welcome to João Coelho's digital realm_</span
            >
          </div>
        </div>
      </div>
    ...
    <div v-if="animationComplete" class="relative z-10 min-h-screen">
      <header class="cyberpunk-header">
        <div class="container mx-auto px-4 py-6">
          <h1
            class="text-4xl md:text-6xl font-bold text-green-400 glitch mb-2"
      ...
      <main class="container mx-auto px-4 py-8">
        <div
          class="cyberpunk-terminal bg-black/70 border border-green-500/30 rounded-lg p-5 relative overflow-hidden"
        >
          ...
          <div v-if="loadingProgress < 100" class="hacking-animation">
            ...
            <div class="access-messages">
              <div
                v-for="(message, index) in accessMessages"
                :key="index"
                :class="{
                  'message-appear': loadingProgress >= (index + 1) * 20,
                }"
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 md:gap-8">
            <div>
              <div>
                <div
                ...
                >
                  <div class="flex items-center gap-2 mb-2.5 font-mono">
                    ...
                    <span class="text-green-500"
                      >cd ./{{ panel.name.toLowerCase() }}</span
                    >
                  </div>
                  <div class="pl-5 border-l border-dashed border-green-500/30">
                    <div class="flex items-center mb-2">
                      <i
                        :class="panel.icon"
                        class="text-2xl mr-3 text-green-400"
                      ></i>
                    ...
                    <div
                      class="inline-block mt-2 px-2 py-1 bg-green-500/10 border border-green-500/30 rounded"
                    >
                      <span class="text-green-500 text-sm"
                        >PRESS TO EXECUTE</span
                      >
                    </div>
                  </div>
                </div>
          ...
          <div class="mt-8 border-t border-dashed border-green-500/30 pt-5">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div
                ...
                :key="index"
                class="bg-black/50 border border-green-500/20 rounded p-2.5"
              >
                <div class="text-xs text-green-500/70 mb-1">
                  {{ stat.name }}
                </div>
    ...
    <div
      ...
      class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/80 p-5 rounded-lg z-50"
    >
      <div
        class="bg-gray-900 p-5 rounded-lg border border-green-500 text-green-500"
      >
