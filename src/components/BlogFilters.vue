<template>
  <div class="space-y-4">
    <div class="relative">
      ...
      />
      <i class="bi bi-search absolute left-3 top-1/2 transform -translate-y-1/2 text-green-500"></i>
    </div>

    <div
      class="space-y-3 md:space-y-0 md:flex md:flex-wrap md:gap-4 md:items-center"
    >
    ...
    <div
      class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-gray-400"
    >
      <span>
        Showing {{ filteredCount }} of {{ totalCount }} posts
        <span v-if="hasActiveFilters" class="text-green-500"> (filtered) </span>
      </span>
